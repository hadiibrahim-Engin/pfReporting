"""PowerFactory-Datenextraktion.

Alle öffentlichen Methoden geben reine Pydantic-Modelle zurück –
kein PowerFactory-Objekt verlässt diese Datei.
"""
...
class PowerFactoryReader:
    """Kapselt alle Zugriffe auf die PowerFactory-Python-API."""
    ...
    def get_project_info(self) -> ProjectInfo:
        ...
        return ProjectInfo(
            project=self._loc_name(project, "Unbekannt"),
            study_case=self._loc_name(sc, "Unbekannt"),
    ...
    def get_switched_elements(self) -> list[SwitchedElement]:
        ...
        type_map = {
            "ElmLne": "Leitung",
            "ElmTr2": "Transformator (2W)",
            "ElmTr3": "Transformator (3W)",
        }
        ...
        log.info("Freigeschaltete Betriebsmittel: %d", len(results))
        return results
    ...
    def get_loadflow_results(self) -> LoadFlowResult:
        ...
        try:
            ...
            log.warning("Lastfluss-Berechnung fehlgeschlagen: %s", exc)
        ...
        log.info(
            "Lastfluss: konvergiert=%s  Last=%.2f MW  Erzeugung=%.2f MW  Verluste=%.2f MW",
            converged,
            total_load,
            total_gen,
            losses,
        ...
        return LoadFlowResult(
            converged=converged,
            status_text="Konvergiert" if converged else "Nicht konvergiert",
    ...
    def get_voltage_results(self) -> list[VoltageResult]:
        ...
        log.info("Spannungsergebnisse: %d Knoten", len(results))
        return results
    ...
    def get_loading_results(self) -> list[LoadingResult]:
        ...
        log.info("Thermische Auslastung: %d Betriebsmittel", len(results))
        return results

    # ── N-1-Analyse ───────────────────────────────────────────────────────

    def get_n1_results(self) -> list[N1Result]:
        """Führt eine manuelle N-1-Analyse durch (ComSimoutage als Fallback)."""
        results = self._try_simoutage()
        if not results:
            results = self._manual_n1()
        log.info("N-1-Analyse: %d Fälle", len(results))
        return results
    ...
    def _try_simoutage(self) -> list[N1Result]:
        try:
            ...
            return []
        return []  # Ergebnisauslese hängt stark von PF-Version ab → Fallback

    def _manual_n1(self) -> list[N1Result]:
        app = self._app
        ldf = app.GetFromStudyCase("ComLdf")
        if ldf is None:
            raise ReaderError("Kein ComLdf-Objekt im Studienfall gefunden")

        candidates = [
            (e, "Leitung") for e in self._calc_objects("*.ElmLne")
        ] + [
            (e, "Transformator (2W)") for e in self._calc_objects("*.ElmTr2")
        ]
    ...
    def _fill_n1_postcontingency(self, entry: N1Result) -> None:
        ...
        if max_load > n1_cfg.max_loading_pct:
            entry.violations.append(f"Überlastung: {max_load_elem} mit {max_load:.1f}%")
        if min_v < n1_cfg.min_voltage_pu:
            entry.violations.append(
                f"Unterspannung: {min_v_node} mit {min_v:.4f} p.u."
            )
        if max_v > n1_cfg.max_voltage_pu:
            entry.violations.append(
                f"Überspannung: {max_v_node} mit {max_v:.4f} p.u."
            )

    # ── Zeitreihen (Quasi-Dynamic) ────────────────────────────────────────

    def load_elmres(self, name: str | None = None):
        sc = self._app.GetActiveStudyCase()
        if not sc:
            raise ReaderError("Kein aktiver Studienfall")
        res_name = name or self._cfg.report.quasi_dynamic_result_file
        res_list = sc.GetContents(res_name) or sc.GetContents("*.ElmRes")
        if not res_list:
            raise ReaderError("Kein ElmRes-Objekt gefunden. Simulation zuerst ausführen.")
        elmres = res_list[0]
        elmres.Load()
        log.info("ElmRes geladen: %s", self._loc_name(elmres))
        return elmres
    ...
    def get_time_series(
        ...
        log.info(
            "Zeitreihen extrahiert: %d Abschnitte, %d Serien, %d Zeitschritte",
            len(sections),
            total_series,
            nrows,
        )
